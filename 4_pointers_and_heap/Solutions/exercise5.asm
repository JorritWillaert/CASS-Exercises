...
# First 4 bytes store either 0 (if the chunk is allocated) or the address of the next free chunk (if the chunk is not allocated, thus part of a free list)
...
skip:
	...
	ret
new_chunk:
	sw zero, (gp)	#Store 0 in the first 4 bytes to denote the fact this chunk is allocated
	sw a0, 4(gp)	#Store the chunk size in the next 4 bytes
	addi gp, gp, 8 #Move the gp by 8 - the actual start of the newly reserved memory
	mv t0, a0		#Back-up the bytes to reserve in t0
	mv a0, gp		#Return the actual start of the newly reserved memory
	add gp, gp, t0 #Reserve bytes by incrementing the gp 
	ret
