...
inner_loop:
	...
	# Load the string characters (notice lb instead of lw because a char is 1 byte in size)
	lb	t3, 0(t2)
	lb	t4, 0(t1)

	# Iterate through the strings by moving the inner loop pointers (notice we increment by 1 instead of 4 now)
	addi	t1, t1, 1
	addi	t2, t2, 1
	...
	# If we have not reached the end of the string, check the next position in the outer loop (step 1 byte)
	addi	t0, t0, 1
	bnez	t3, outer_loop
